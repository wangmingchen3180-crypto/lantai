...
叫法走水象，和涟漪、澜台同一套意象。**定案（2026-08-14 用户拍板）：**

| 位置 | important | urgent | 名称 | 一行说明 |
| --- | --- | --- | --- | --- |
| 左上 | 是 | 是 | 急流 | 卷着走，先过这一段 |
| 右上 | 是 | 否 | 深潭 | 要沉下去做，别搁浅 |
| 左下 | 否 | 是 | 急滩 | 水急但浅，蹚过去就完 |
| 右下 | 否 | 否 | 静水 | 没动静，想不起来也无妨 |

四个名字不是孤立的雅词，两条轴都写在字面上：

- **左列共用「急」字**（急流 / 急滩），右列是不急的水体（深潭 / 静水）。急这条轴直接可读。
- **上排是有深度的水**（急流 / 深潭 —— 流与潭都成体量），下排是浅的、没份量的（急滩 / 静水）。要紧这条轴靠深浅对照。

弃用记录，别再提回来：

- ~~`现在做` / `早点做` / `顺手做` / `先放着`~~ —— 准确但没味道，和「澜」不搭。
- ~~`浅滩`~~（原候选里的左下）—— 显不出「急」。左列两格都该是急的，只有 `急流` 带急字就废了半条轴。换成 `急滩`（真词，水急而浅）后左列统一以急开头。
- ~~`急重` / `徐重` / `急轻` / `徐轻`~~ —— 备选方案，名字即坐标、零翻译，但读起来像标签不像产品语言。若日后觉得水象太绕，这是退路。

**硬条件：** 网格边缘必须标出两条轴（上排写「要紧」、左列写「急」），每格下面必须有那行说明。诗意的名字只能是**结构之上的装饰**，不能替代结构——否则用户扔一条待办时要停下来翻译「深潭是什么意思」，那一下停顿就是成本。已知最弱的一处是 `深潭` 与 `静水` 的区分度（都静，差别在深与无），靠轴标签和说明兜。
