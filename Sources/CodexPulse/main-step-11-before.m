...
static const CGFloat CPOrbPeek = 36.0;
...
- (void)buildWindow {
    ...
    pill.wantsLayer = YES;
    pill.layer.backgroundColor = CPSurface().CGColor;
    pill.layer.cornerRadius = CPOrbSize / 2.0;
    pill.layer.borderWidth = 1.0;
    pill.layer.borderColor = CPBorder().CGColor;
    self.pill = pill;
    self.window.contentView = pill;

    NSShadow *shadow = [[NSShadow alloc] init];
    shadow.shadowColor = [NSColor colorWithSRGBRed:0.078 green:0.078 blue:0.086 alpha:0.10];
    shadow.shadowOffset = NSMakeSize(0, 4);
    shadow.shadowBlurRadius = 18.0;
    pill.shadow = shadow;

    self.iconView = [[NSImageView alloc] initWithFrame:NSMakeRect(14, 14, 24, 24)];
    self.iconView.image = CPSymbol(@"circle.grid.cross.fill", 20, CPAccent());
    self.iconView.contentTintColor = CPAccent();
    [pill addSubview:self.iconView];

    self.badgeView = [[NSImageView alloc] initWithFrame:NSMakeRect(32, 32, 18, 18)];
    self.badgeView.image = CPDotImage(18, CPAccent());
    self.badgeView.hidden = YES;
    [pill addSubview:self.badgeView];

    self.badgeLabel = [[NSTextField alloc] initWithFrame:NSMakeRect(32, 32, 18, 18)];
    self.badgeLabel.stringValue = @"";
    self.badgeLabel.font = [NSFont systemFontOfSize:9 weight:NSFontWeightBold];
    ...
    [pill addSubview:self.badgeLabel];
...
- (void)applyFrame {
    ...
    [self updatePillShape];
}

- (void)updatePillShape {
    if (self.docked) {
        self.pill.layer.cornerRadius = 0;
        self.pill.layer.backgroundColor = CPSurface().CGColor;
    } else {
        self.pill.layer.cornerRadius = CPOrbSize / 2.0;
        self.pill.layer.backgroundColor = CPSurface().CGColor;
    }
}
...
- (void)peek:(BOOL)show {
    ...
    CGFloat x;
    if (self.dockEdge == NSRectEdgeMaxX) {
        x = NSMaxX(visible) - CPOrbPeek;
    } else {
        x = NSMinX(visible);
    }
    [self.window setFrame:NSMakeRect(x, y, w, h) display:YES];
    self.pill.layer.cornerRadius = 8.0;
    if (self.dockEdge == NSRectEdgeMaxX) {
        self.pill.layer.maskedCorners = kCALayerMinXMinYCorner | kCALayerMinXMaxYCorner;
    } else {
        self.pill.layer.maskedCorners = kCALayerMaxXMinYCorner | kCALayerMaxXMaxYCorner;
    }
}
...
- (void)mouseEntered:(NSEvent *)event { [self peek:YES]; }
- (void)mouseExited:(NSEvent *)event { [self unpeek]; }
...
- (void)pillMouseDown:(NSEvent *)event {
    ...
    if (self.docked) {
        self.docked = NO;
        self.freeX = self.dragStartOrigin.x;
        self.freeY = self.dragStartOrigin.y;
        [self applyFrame];
    }
    self.pill.layer.transform = CATransform3DMakeScale(0.95, 0.95, 1.0);
}
