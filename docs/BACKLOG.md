...
**叫法待拍板**，判据已定：四个特征字必须互不相同，且竖着能分出动静、横着能分出深薄。领先方案 `急流` / `深潭` / `碎浪` / `静水`（急·深·碎·静），备选是完全对称的 `急流`/`缓流`/`急滩`/`缓滩`。弃用 ~~`现在做`/`早点做`/`顺手做`/`先放着`~~（没味道）、~~`浅滩`~~（显不出急）、~~`急滩`~~（与急流共用一字，四格只剩三个特征字）。详见 [`design-systems/lantai/DESIGN.md`](../design-systems/lantai/DESIGN.md)。
