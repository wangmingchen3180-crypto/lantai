...
逻辑是一个开关，只有两个状态：四格态各自显示条目；点一格**满格接管，其余三格完全消失**（不是缩成窄条）；再点一次缩回；换格必须先缩回；点条目即完成。满格态只干一件事——录入，默认焦点在输入框，不堆筛选和排序。

完整规格（含手机端表现与空间账）见 [`design-systems/lantai/DESIGN.md`](../design-systems/lantai/DESIGN.md) 的四象限一节。

**位置是通行约定**，重要在上排、紧急在左列，从左上往右下读即优先级顺序。用户给的参考图是举例，不作为依据。

**叫法已定（2026-08-14）**：`急流`（左上）/ `深潭`（右上）/ `碎浪`（左下）/ `静水`（右下）。判据是四个特征字互不相同、且竖着分出动静（流浪在动、潭水不动）、横着分出深薄（流潭有体量、浪与静水只在表面）；`碎` 直通中文说小事的「琐碎」。弃用 ~~`现在做`/`早点做`/`顺手做`/`先放着`~~（没味道）、~~`浅滩`~~（显不出急）、~~`急滩`~~（与急流共用一字，四格只剩三个特征字）。详见 [`design-systems/lantai/DESIGN.md`](../design-systems/lantai/DESIGN.md)。
