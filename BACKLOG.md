...
- 在 Codex Pulse 中增加独立的 Todo/待办能力，让工作台不仅查看 Agent 任务，也能承接后续需要人工完成的事项。
- 支持手动新增、编辑、完成和归档待办，并预留截止时间、优先级和项目归属。
- 支持把 Codex 任务中的行动项或未完成事项转成 Todo，并保留与原 Agent、原任务的关联及直达入口。
- 后续确认数据源方案：应用内本地待办、Apple 提醒事项、Obsidian 或其他外部 Todo 服务；在确定前不绑定具体平台。
- Todo 数量与“需关注任务”提示气泡应分别表达，避免用户无法判断数字代表 Agent 状态还是个人待办。
- 评估双向完成同步：Todo 完成是否影响 Agent 任务，以及 Agent 继续工作或完成后是否自动更新关联 Todo。
