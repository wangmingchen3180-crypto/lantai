...
#### 交互：四格总览 + 点一格接管（已定，2026-08-14）

硬摊 2×2 装不进现在的待办栏（展开态列表区 520×158，每格只剩两行高）。用户参考了一套手机设计的交互逻辑（**只取交互，不取视觉**——配色一律按澜台的深石墨走，不做白底大留白那套）：四格总览，点一格胀开接管整个区域，直接录入，录完返回。

价值在于「记一条的时候不被另外三格打扰」。所以接管态默认焦点就在输入框，那一屏不堆筛选器和排序按钮。

位置、叫法、以及为什么这么排，见 [`design-systems/lantai/DESIGN.md`](../design-systems/lantai/DESIGN.md) 的四象限一节（定案：左上现在做 / 右上早点做 / 左下顺手做 / 右下先放着；重要在上排、紧急在左列，从左上往右下读就是该先看哪个的顺序）。

象限标签用三字祈使句而不是诗意名字：标签的唯一职责是让人扔待办时不用思考扔哪格，诗意会让人停顿去翻译。澜台的调性放在每格下面那行说明里。
