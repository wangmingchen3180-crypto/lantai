...
对你**正在用的** Codex Desktop 和 Kimi 窗口，澜台是纯只读的：只读它们的本地索引，不写它们的任何文件，不代你在那些会话里回话或点同意。它挂了，你的 Agent 照跑。

这是设计，不是没做完。同类工具的常见做法是把 Agent 装进自己的终端里托管起来，那样能看得更深，代价是你必须住在它里面。澜台反过来：你照常用原生 App，它只在旁边看着。

另有一条独立的路径：澜台也能**自己拉起** Codex 任务（走 `codex app-server`）。这类任务叫托管任务，可以从手机新建、中途插话和打断。托管任务只包括澜台自己起的那些——你在 Codex Desktop 里开的会话，它指挥不了，只能看。

不想要这条路径就不用管它：手机连接默认关闭，关着的时候澜台不监听任何端口。
...
同一个 WiFi 下扫码配对，手机上能看任务进度，也能给托管任务发指令。

**默认关闭。** 菜单栏 →「手机连接（局域网）」打开之后，澜台才会在 `8787` 起一个 HTTP 服务；关掉就不再监听。

几条边界，装之前先看清楚：

- 明文 HTTP，没有 TLS。只适合家里和私有组网（比如 Tailscale）。不要在公共 WiFi 上开着，更不要往公网映射端口。
- 新配对的设备默认**只能看**。指挥权限要在「手机指挥设置」里逐台手动打开。
- 手机能动的目录，限定在你手动加进白名单的项目里。
- 手机只能指挥托管任务，碰不到你在 Codex Desktop 里开的会话。
- 配对码 6 位、3 分钟过期、连错 5 次锁 10 分钟。token 存 Keychain，可随时撤销。

接口细节见 [docs/BRIDGE_API.md](docs/BRIDGE_API.md)。
...
- SQLite 一律只读连接
- 不读 API key；不改对方的任何文件
- 事件流只读有界尾部，不整个加载
- 任务观察本身不发网络请求
- 额度是另开的一条：Codex 经本机已在跑的 `codex app-server` 代取；Kimi 只读桌面端 Cookies 里的 `kimi-auth`，再向 kimi.com 查用量。读不到就显示「额度不可用」，不把失败画成 0%。不上传任务内容。
- 手机连接关着的时候不监听任何端口；打开后只绑局域网，不做 UPnP、不打洞、不经过任何第三方服务器
- 澜台会起一个 `codex app-server` 子进程用来读额度和跑托管任务。它走标准输入输出，不监听网络
...
| [docs/BRIDGE_API.md](docs/BRIDGE_API.md) | 手机端与 Mac 之间的接口协议 |
