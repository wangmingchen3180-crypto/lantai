...
- (void)buildHeader {
    ...
    NSButton *modeButton = CPIconButton(@"rectangle.2.swap", self, @selector(toggleDockMode:), @"切换浮钮形态");
    ...
    NSStackView *right = [NSStackView stackViewWithViews:@[railToggle, detailToggle, modeButton, self.pinButton, closeButton]];
...
- (void)updateCardWidth {
    ...
    f.size.width = w;
    self.card.frame = NSMakeRect(0, 0, w, CPCardHeight);
...
- (NSRect)targetFrameNearDockRect:(NSRect)rect edge:(NSRectEdge)edge {
    ...
    return NSMakeRect(x, y, w, CPCardHeight);
}
...
- (void)buildBarView {
    ...
    NSButton *logo = [NSButton buttonWithTitle:@"" target:self action:@selector(barLogoClicked:)];
    logo.bordered = NO;
    logo.image = CPSymbol(@"circle.grid.cross.fill", 17, CPAccent());
    logo.imagePosition = NSImageOnly;
    logo.translatesAutoresizingMaskIntoConstraints = NO;
    [logo.widthAnchor constraintEqualToConstant:32].active = YES;
    ...
    [NSLayoutConstraint activateConstraints:@[
        ...
        [agents.leadingAnchor constraintEqualToAnchor:logo.trailingAnchor constant:4],
...
- (void)renderDockAgents {
    ...
    for (CPAgent *agent in self.agents) {
        NSButton *btn = [NSButton buttonWithTitle:@"" target:self action:@selector(barAgentClicked:)];
        btn.bordered = NO;
        ...
        [btn.widthAnchor constraintEqualToConstant:CPBarItem].active = YES;
        [btn.heightAnchor constraintEqualToConstant:CPBarItem].active = YES;
        [self.barAgentStack addArrangedSubview:btn];
    }
    CGFloat width = 8 + 32 + 4 + MAX(self.agents.count, 1) * CPBarItem + (MAX(self.agents.count, 1) - 1) * 2 + 8;
...
- (void)setMode:(NSInteger)mode {
    ...
    NSScreen *screen = self.targetScreen;
    NSRect visible = screen.visibleFrame;
    if (mode == 1 && (self.freeX == 0 || self.freeY == 0)) {
